function ardy = SensaStim_Ardy_Connect (varargin)
...
if isempty(port)                                                            %If no serial ports were found on this computer...
    error('ERROR IN CONNECTION FUNCTION: There are no available serial ports on this computer.'); %Show an error.
end
...
if isempty(port)                                                            %If no port was selected.
    warning(['ERROR IN CONNECTION FUNCTION:NoPortChosen','No serial port chosen '...
        'for Ardy. Connection to the Arduino was aborted.']);     %Show a warning.
...
try                                                                         %Try to open the serial port for communication.
    ...
    error(['ERROR IN CONNECTION FUNCTION: Could not open a serial '...
...
ardy.read_report = @()read_report(serialcon);
...
function output = read_report (serialcon)
    output = -1;
    if (serialcon.BytesAvailable > 0)
        try
            report_str = fgetl(serialcon);
            output = str2double(report_str);
        catch
            % empty
        end
    end
